...
function setTripStats(tripInfos) {
  return new Promise((resolve, reject) => {
    ...
    if (tripInfos !== undefined) {
      let bikeTripInfos = tripInfos.filter(
        (tripInfo) => tripInfo.locomotion === "bike"
      );
...
function loadTrips(tripInfos, detailLevel) {
  return new Promise((resolve, reject) => {
    tripInfos.forEach((tripInfo) => {
      getTripGeoJson(tripInfo, detailLevel).then(
        (geoJson) => {
...
function getLastPosition() {
  return new Promise((resolve, reject) => {
    realtimeDB.ref("last-position").once(
      "value",
      (snapshot) => {
...
function startLiveTracking() {
  ...
  realtimeDB.ref("last-position").on("value", liveTrackingCallback);
}

function stopLiveTracking() {
  realtimeDB.ref("last-position").off("value", liveTrackingCallback);
}

function setBikeIcon() {
  getLastPosition().then((lastPos) => {
    bikeMarker.setLatLng([lastPos.lat, lastPos.lng]);
  });
}
...
L.tileLayer(
  "https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}",
  {
    ...
    accessToken:
      "pk.eyJ1IjoidGhvbXNhbm8iLCJhIjoiY20xajRmNG1qMHYxaDJrc2VxcnhnNmVhaiJ9.cR43N9bkoAtmIwM-Yd4PfA",
    dashArray: "20,15",
  }
).addTo(map);
...
var photoLayer = L.photo.cluster().on("click", function (evt) {
  ...
  if (photo.video) {
    template =
      '<iframe width="480" height="270" src={video}></iframe></a><p>{caption}</p>';
  }
