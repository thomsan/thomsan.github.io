...
gulp.task("build:styles-main", function () {
  var cssFiles = gulp.src(paths.appMainCssFilesGlob).on("data", (file) => {
    console.log("CSS File:", file.path);
  });
  var scssFiles = gulp
    .src(paths.appMainScssFilesGlob)
    .pipe(sass().on("error", gutil.log));

  return es
    .concat(cssFiles, scssFiles)
...
gulp.task("build:scripts-firebase", function () {
  return gulp
    .src(paths.appFirebaseJsFilesGlob)
...
gulp.task("clean:scripts", function (cb) {
  del([
    paths.jekyllDir + "main.js",
    paths.siteDir + "main.js",
    paths.jekyllDir + "map.js",
    paths.siteDir + "map.js",
    paths.jekyllDir + "firebase.js",
    paths.siteDir + "firebase.js",
  ]);
  cb();
});
...
gulp.task("build:jekyll", function () {
  var shellCommand =
    "jekyll build --config _config.yml,_app/localhost_config.yml";
  if (config.drafts) {
    shellCommand += " --drafts";
  }
...
gulp.task(
  "build:jekyll:watch",
  gulp.series("build:jekyll", function (cb) {
    browserSync.reload();
    cb();
  })
);
gulp.task(
  "build:scripts:watch",
  gulp.series("build:scripts", function (cb) {
    browserSync.reload();
    cb();
  })
);
...
gulp.task(
  ...
  gulp.series("build", function () {
    ...
    gulp.watch(
      paths.jekyllDir + "Blog/**/*.+(md|markdown|MD)",
      gulp.series("build:jekyll:watch")
    );
    ...
    if (config.drafts) {
      gulp.watch(
        paths.jekyllDir + "_drafts/*.+(md|markdown|MD)",
        gulp.series("build:jekyll:watch")
      );
    }
    ...
    gulp.watch(
      paths.jekyllDir + "_data/**.*+(yml|yaml|csv|json)",
      gulp.series("build:jekyll:watch")
    );

    // Watch Jekyll favicon.ico
    gulp.watch(
      paths.jekyllDir + "favicon.ico",
      gulp.series("build:jekyll:watch")
    );
